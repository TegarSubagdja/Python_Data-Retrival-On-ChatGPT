...
# DATA
# ==================================================
def remove_none_code(tokens: list) -> list:
    result_tokens = []
    for token in tokens:
...
def parsing_and_checking(text: str, name:str, code:str) -> bool:
    text = preprocessing(text)
    text_arr = text.split()
    text_arr = remove_none_code(text_arr)

    len_text = len(text_arr)

    if code:
        code = preprocessing(code)
        code_arr = sorted(code.split())
        code_arr = remove_none_code(code_arr)
        len_code = len(code_arr)
        code_str = " ".join(code_arr)
        
        for idx in range(len_text - len_code + 1):
            window_code = sorted(text_arr[idx:idx + len_code])

            window_str = " ".join(window_code)

            if window_str == code_str:
                return True

    if name:
        name = preprocessing(name)
        name_arr = sorted(name.split())
        name_arr = remove_none_code(name_arr)
        len_name = len(name_arr)
        name_str = " ".join(name_arr)

        for idx in range(len_text - len_name + 1):
            window_code = sorted(text_arr[idx:idx + len_name])

            window_str = " ".join(window_code)

            if window_str == name_str:
                return True
...
def save_result(
    ...
    if parsing_and_checking(text=text, name=name, code=code):
